"""
Intent方式は廃止されました。
dialogue_flow.py を使用してください。

後方互換性のため、以下の関数は残しています:
- normalize_text: テキスト正規化
- interpret_handoff_reply: ハンドオフ確認時のYES/NO判定
- get_response_template: テンプレート取得
- get_template_config: テンプレート設定取得
- TEMPLATE_CONFIG: テンプレート設定辞書
"""
...
TEMPLATE_CONFIG: dict[str, dict] = {
    ...
    "120": {"text": "導入について、どの点が気になっていますか？", "voice": "ja-JP-Neural2-B", "rate": 1.1},
    "121": {"text": "24時間365日対応しております。", "voice": "ja-JP-Neural2-B", "rate": 1.1},
    "122": {"text": "料金について、月額料金、初期費用、通話料、解約料など、どの項目についてお聞きになりたいでしょうか？", "voice": "ja-JP-Neural2-B", "rate": 1.1},
...
def get_template_config(template_id: str) -> dict | None:
    """テンプレートIDに対応する設定辞書を返す"""
    return TEMPLATE_CONFIG.get(template_id)


# 後方互換性のためのダミー関数
def classify_intent(text: str, context: str | None = None) -> str:
    """Intent方式は廃止されました。常にUNKNOWNを返します。"""
    logger.warning(f"classify_intent is deprecated. text='{text}', context={context}")
    return "UNKNOWN"


def select_template_ids(intent: str, text: str) -> list[str]:
    """Intent方式は廃止されました。空リストを返します。"""
    logger.warning(f"select_template_ids is deprecated. intent='{intent}', text='{text}'")
    return []
