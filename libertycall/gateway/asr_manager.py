...
class GatewayASRManager:
    ...
    async def process_rtp_audio(self, data: bytes, addr: Tuple[str, int]):
        ...
        if self._payload_raw_debug_count < 5 and len(pcm_data) > 0:
            self.logger.warning(
                f"[PAYLOAD_RAW] Cnt={self._payload_raw_debug_count} Len={len(pcm_data)} Head={pcm_data[:10].hex()}"
            )
        ...
        if effective_call_id:
            ...
            if is_voice:
                ...
                if effective_call_id in self._last_voice_time:
                    ...
                    if silence_duration >= 2.0:
                        ...
                        if not self._backchannel_flags.get(effective_call_id, False):
                            ...
                            try:
                                if (
                                    hasattr(self.ai_core, "tts_callback")
                                    and self.ai_core.tts_callback
                                ):
                                    self.ai_core.tts_callback(
                                ...
                                self.logger.exception(
                                    f"[BACKCHANNEL_ERROR] call_id={effective_call_id} error={e}"
                                )
                ...
                if self._silent_frame_count % 100 == 0:
                    self.logger.debug(
                        f"[RTP_SILENT] Detected silent frame (RMS < {threshold}) count={self._silent_frame_count}"
                    )
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"[ULAW_ERROR] call_id={effective_call_id} ulaw2lin failed: {e}"
                )
                pcm16_8k = b""
            ...
            if self._debug_packet_count <= 50 or self._debug_packet_count % 100 == 0:
                ...
                self.logger.warning(
                    f"[AUDIO_DEBUG] Cnt={self._debug_packet_count} RawHead={raw_preview} "
