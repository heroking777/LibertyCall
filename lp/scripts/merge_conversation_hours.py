...
# -*- coding: utf-8 -*-
"""
LibertyCall デモ音声合成スクリプト（パターン2：営業時間のご案内）
...
# --- ファイル定義 ---
CUSTOMER_PATH = AUDIO_DIR / "12月13日（20-29）.m4a"
AI_VOICE_FILES = [
    "ai_voice_hours_01.mp3",
    "ai_voice_hours_02.mp3",
    "ai_voice_hours_03.mp3",
    "ai_voice_hours_04.mp3",
]
OUTPUT_FILE = AUDIO_DIR / "demo_hours.mp3"
...
section_durations = [
    customer_duration * 0.23,  # 最初の応答（営業時間を教えて）
    customer_duration * 0.38,  # 2番目（さらに長め - AI音声3の前に人間が話し終わるように）
    customer_duration * 0.28,  # 3番目（さらに長め - AI音声4の前に人間が話し終わるように）
    customer_duration * 0.11,  # 最後（余り）
]
...
for i, duration in enumerate(section_durations):
    ...
    # 最初のセクションは1秒後から、セクション2は3秒以上チェック、セクション3は控えめに、セクション4は積極的に削除
    if i == 0:
        check_range = 1000
    elif i == 1:
        ...
        # セクション3は無音削除を控えめに（1.5秒まで）
        check_range = 1500
    else:
        # セクション4は無音が長いので、積極的に削除（3秒までチェック）
        check_range = 3000
