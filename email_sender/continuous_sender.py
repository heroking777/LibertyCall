...
from email_sender.pre_send_verify import verify_email
...
class ContinuousSender:
    ...
    def send_single_email(self, recipient: Dict) -> bool:
        ...
        # 送信前SMTP検証
        try:
            can_send, reason = verify_email(email)
            if not can_send:
                logger.info(f"SMTP検証で除外: {email} ({reason})")
                recipient["除外"] = f"smtp_verify_{reason}"
                return False
        except Exception as e:
            logger.warning(f"SMTP検証エラー（送信続行）: {email} - {e}")
