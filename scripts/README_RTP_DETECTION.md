...
### 1-2. `detect_rtp_port_enhanced.sh` - 手動実行版（改良版）

基本版を改良し、接続の安定化、エラーハンドリング、再接続機能を強化したバージョンです。

### 1-3. `detect_rtp_port_final.sh` - 手動実行版（最終版）⭐⭐ 最推奨

改良版をさらに改善し、**接続待ちループ + 再接続対応 + セッション維持**を実装した最終版です。

**主な改善点:**
- **接続待ちループ**: Event Socket接続可能になるまで自動待機（最大30秒）
- **セッション維持**: 接続を保持して複数コマンドを連続実行（`-x`ではなく接続保持モード）
- **再接続機能**: 失敗時に自動的に再接続モード（`-r`）で再試行
- 双方向RTP確認用のtcpdumpコマンドを自動生成

**使い方:**
```bash
# 通話開始後、すぐに実行（最推奨）
/opt/libertycall/scripts/detect_rtp_port_final.sh
```

**特徴:**
- Event Socket接続可能になるまで自動待機（接続タイミング問題を解決）
- 接続を保持して複数コマンドを連続実行（セッション維持）
- 失敗時の自動再接続（`-r`オプション使用）
...
### 2-2. `auto_detect_rtp_enhanced.sh` - 完全自動化版（改良版）

基本版を改良し、接続の安定化、エラーハンドリング、再接続機能を強化したバージョンです。

### 2-3. `auto_detect_rtp_final.sh` - 完全自動化版（最終版）⭐⭐ 最推奨

改良版をさらに改善し、**接続待ちループ + 再接続対応 + セッション維持**を実装した最終版です。

**主な改善点:**
- **接続待ちループ**: Event Socket接続可能になるまで自動待機（最大30秒）
- **セッション維持**: 接続を保持して複数コマンドを連続実行
- **定期的な接続チェック**: 10回ごとに接続状態を確認し、切断時は自動再接続
...
### ケース1: 手動で1回だけ取得したい場合（最推奨：最終版）

```bash
# 通話開始後、すぐに実行（最終版：接続待ちループ + セッション維持）
/opt/libertycall/scripts/detect_rtp_port_final.sh
...
### ケース2: 継続的に監視したい場合（最推奨：最終版）

```bash
# ターミナル1: 監視スクリプトを実行（最終版：接続待ちループ + セッション維持）
/opt/libertycall/scripts/auto_detect_rtp_final.sh
...
**最終版の特徴:**
- Event Socket接続可能になるまで自動待機
- 定期的な接続チェック（10回ごと）と自動再接続
- 接続を保持して複数コマンドを連続実行
...
- **接続待ちループ**: Event Socket接続可能になるまで自動待機（最大30秒）
  - これにより、`Error Connecting []` の接続タイミング問題を根本的に解決
- **セッション維持**: 接続を保持して複数コマンドを連続実行（`-x`ではなく接続保持モード）
  - これにより、短時間通話でも確実に情報を取得可能
- **自動再接続機能**: 失敗時に自動的に再接続モード（`-r`）で再試行
- **定期的な接続チェック**: 自動検出版では、10回ごとに接続状態を確認
- **エラーカウント**: 連続したエラーをカウントし、一定回数以上で再接続を試みます
