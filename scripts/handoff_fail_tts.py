...
# -*- coding: utf-8 -*-
"""
転送失敗時のTTSアナウンスをGateway経由で送信するスクリプト
GatewayのAICoreを直接呼び出してTTSを生成し、ログに記録
実際のRTP送信はGatewayが行う（将来的な実装）
"""
...
def send_handoff_fail_tts(call_id: str = "TEMP_CALL"):
    """
    転送失敗時のTTSアナウンスを生成してGatewayに通知
    
    注意: 現在の実装では、Gatewayのログファイルにメッセージを書き込む
    将来的には、GatewayのAPIを使用してTTSアナウンスを送信する
    ...
    try:
        ...
        print(f"HANDOFF_FAIL_TTS: TTS generated successfully (len={len(ulaw_response)})")
        print(f"WARNING: Direct RTP sending not implemented. Gateway API required.")
        return True
...
if __name__ == "__main__":
    ...
    success = send_handoff_fail_tts(call_id)
