...
INPUT_CSV = '/opt/libertycall/scraper/data/gbizinfo_test50.csv'
OUTPUT_CSV = '/opt/libertycall/scraper/output/searxng_emails_test50.csv'
DONE_FILE = '/opt/libertycall/scraper/data/searxng_done_test50.txt'
    ...
    with open(INPUT_CSV, 'r') as f:
        for line in f:
            ...
            if domain:
                found_domains += 1
                # SMTP推測
                emails = extract_emails(url, deep_search=False, smtp_guess=True)
                for email in emails:
                    found_emails += 1
                    writer.writerow({
                        'email': email,
